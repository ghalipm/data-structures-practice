...
public class MyTree {
    ...
    // task -5B sum Node values of a BST
    public int findSumOfNodes(Node root){
        if (root==null) return 0;
        return root.value + findSumOfNodes(root.leftChild) + findSumOfNodes(root.rightChild);
    }

    // task -6 return height in a BST
    public int heightOfTree(Node root){
        if(root==null) return -1;
        if (isLeaf(root)) return 0;
        return Math.max(heightOfTree(root.leftChild), heightOfTree(root.rightChild))+1;
    }
